...
export interface HALClient {
    ...
    getResource<T extends HALResource | undefined>(link: Link): T;
...
export function useHALClient(): HALClient {
    ...
    const go = async function <T extends HALResource>(
        ...
        const cachedResource = resources[link.href];
    ...
    const refresh = async function (links: Link[]) {
        for (const index in links) {
            console.log('Refreshing', links[index]);
            console.log(resources);
            const resource = resources[links[index].href];
            if (!resource) continue;

            await go({
                link: resource.resource._links['self'],
                cache_secs: resource.expires_in_secs,
                ignore_cache: true,
            });
        }
    };
